...
class ParticularDonorVC: UIViewController, UITableViewDelegate, UITableViewDataSource {
    ...
    override func viewDidLoad() {
        ...
        Firestore.firestore().collection("Donors").document(documentId).getDocument { (snapshot, error) in
            ...
            self.callBtn.translatesAutoresizingMaskIntoConstraints = false
            self.callBtn.isUserInteractionEnabled = true
            self.callBtn.leadingAnchor.constraint(equalTo: self.view.leadingAnchor, constant: 20.0).isActive = true
            self.callBtn.trailingAnchor.constraint(equalTo: self.view.trailingAnchor, constant: -20.0).isActive = true
            self.callBtn.heightAnchor.constraint(equalToConstant: 50.0).isActive = true
            self.callBtn.topAnchor.constraint(equalTo: self.lineView.bottomAnchor, constant: 20.0).isActive = true
            self.callBtn.addTarget(self, action: #selector(self.call(_:)), for: .touchUpInside)
            
            self.view.addSubview(self.tableView)
            self.tableView.translatesAutoresizingMaskIntoConstraints = false
            self.tableView.isUserInteractionEnabled = true
            self.tableView.topAnchor.constraint(equalTo: self.callBtn.bottomAnchor, constant: 10.0).isActive = true
